...
namespace IPS\dcudash\setup\upg_100041;
...
{
	/**
	 * Fix Dashes blocks that use topicFeed block
	 *
	...
	public function step1()
	{
		foreach ( \IPS\Db::i()->select( '*', 'dcudash_blocks', array( 'block_type=?', 'plugin' ) ) as $block )
		{
			...
			if ( $block['block_plugin'] == 'LatestDashboards' )
			{
				$block['block_plugin'] = 'RecordFeed';
				if ( isset( $config['database'] ) )
				{
					$config['dcudash_rf_database'] = $config['database'];
			...
			if ( $update )
			{
				\IPS\Db::i()->update( 'dcudash_blocks', array( 'block_plugin' => $block['block_plugin'], 'block_plugin_config' => json_encode( $config ) ), array( 'block_id=?', $block['block_id'] ) );
			}
		}
