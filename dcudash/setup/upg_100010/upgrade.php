...
namespace IPS\dcudash\setup\upg_100010;
...
class _Upgrade
{
	...
	public function step1()
	{
		...
		foreach( \IPS\Db::i()->select( '*', 'dcudash_databases' ) as $db )
		{
			foreach( \IPS\Lang::languages() as $id => $lang )
			{
				try
				{
					$base = $lang->get('dcudash_create_menu_records_x');
				}
			...
			\IPS\Lang::saveCustom( 'dcudash', "dcudash_create_menu_records_" . $db['database_id'], $menu );
		}
	...
	public function step2()
	{
		foreach( \IPS\Db::i()->select( '*', 'dcudash_databases' ) as $db )
		{
			...
			{
				$definition	= \IPS\Db::i()->getTableDefinition( 'dcudash_custom_database_' . $db['database_id'] );
				$fields		= array();
			...
			if( count( $updates ) )
			{
				\IPS\Db::i()->update( 'dcudash_databases', $updates, array( 'database_id=?', $db['database_id'] ) );
			}
		}
	...
	/**
	 * Step 3
	 * Convert dash titles to translatable lang strings
	 *
	 * @return	array	If returns TRUE, upgrader will proceed to next step. If it returns any other value, it will set this as the value of the 'extra' GET parameter and rerun this step (useful for loops)
	 */
	public function step3()
	{
		...
		{
			\IPS\Lang::saveCustom( 'dcudash', "dcudash_dash_" . $dash['dash_id'], $dash['dash_name'] );
		}
	...
	{
		return "Converting dash titles to translatable fields";
	}
}