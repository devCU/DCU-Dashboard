...
namespace IPS\dcudash\setup\upg_101105;
...
class _Upgrade
{
	...
	public function step1()
	{
		...
		foreach( \IPS\Db::i()->select( '*', 'dcudash_templates', array( "template_content LIKE '%settings.reputation_enabled%'" ) ) as $row )
		{
			if ( preg_match( '#\$record\s+?instanceof\s+?\\\IPS\\\Content\\\Reputation#i', $row['template_content'] ) )
			{
				...
				\IPS\Db::i()->update( 'dcudash_templates', array( 'template_content' => $row['template_content'] ), array( 'template_id=?', $row['template_id'] ) ); 
			}
		}
