...
namespace IPS\dcudash\setup\upg_101000;
...
class _Upgrade
{
	...
	{
		foreach ( \IPS\Db::i()->select( '*', 'dcudash_blocks', array( 'block_type=?', 'plugin' ) ) as $block )
		{
			...
			if ( $block['block_plugin'] == 'RecordFeed' )
			{
				...
				foreach ( $config as $_k => $_v )
				{
					if ( $_k !== 'dcudash_rf_database' )
					{
						$_k = str_replace( 'dcudash_rf', 'widget_feed', $_k );
					}
			...
			if ( $update )
			{
				\IPS\Db::i()->update( 'dcudash_blocks', array( 'block_plugin_config' => json_encode( $newConfig ) ), array( 'block_id=?', $block['block_id'] ) );
			}
		}
		
		/* Update template params */
		\IPS\Db::i()->update( 'dcudash_blocks', array( 'block_template_params' => '$records, $title, $orientation=\'vertical\'' ), array( 'block_plugin=?', 'RecordFeed' ) );
