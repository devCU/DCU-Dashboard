...
namespace IPS\dcudash\setup\upg_103000;
...
class _Upgrade
{
	...
	public function step1()
	{
		...
		{
			if ( \IPS\Db::i()->checkForTable( 'dcudash_custom_database_' . $database->id ) AND !\IPS\Db::i()->checkForColumn( 'dcudash_custom_database_' . $database->id, 'record_meta_data' ) )
			{
				...
				$queries[]	= array(
					...
					'query'	=> "ALTER TABLE `" . \IPS\Db::i()->prefix . "dcudash_custom_database_" . $database->id . "` ADD COLUMN " . $column
				);
			}
		}
	...
	{
		\IPS\Task::queue( 'core', 'RebuildSearchIndex', array( 'class' => 'IPS\dcudash\Dashes\DashItem' ), 5 );
	...
	{
		return 'Adding dashes to search index';
	}
	...
	public function finish()
	{
		...
		{
			$ids[] = $id;
			\IPS\Task::queue( 'core', 'RebuildItemCounts', array( 'class' => 'IPS\dcudash\Records' . $id, 'count' => 0 ), 4, array( 'class' ) );
		}
	}
