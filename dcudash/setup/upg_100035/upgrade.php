...
namespace IPS\dcudash\setup\upg_100035;
...
class _Upgrade
{
	...
	{
		$areas = array( 'core_widget_areas', 'dcudash_dash_widget_areas' );
		...
		foreach ( $areas as $table )
		{
			foreach ( \IPS\Db::i()->select( '*', $table ) as $area )
			{
				...
				$whereClause = $table == 'core_widget_areas' ? array( 'id=? AND area=?', $area['id'], $area['area'] ) : array( 'area_dash_id=? AND area_area=?', $area['area_dash_id'], $area['area_area'] );
				...
				{
					if ( $widget['key'] == 'LatestDashboards' )
					{
						$widgets[ $k ]['key'] = 'RecordFeed';
						if ( isset( $widgets[ $k ]['configuration']['database'] ) )
						{
							$widgets[ $k ]['configuration']['dcudash_rf_database'] = $widgets[ $k ]['configuration']['database'];
