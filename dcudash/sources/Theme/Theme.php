...
class _Theme extends \IPS\Theme
{
	...
	public static function i()
	{
		...
		{
			return new \IPS\dcudash\Theme\Advanced\Theme;
		}
		else
		{
			return new self;
	...
	public static function importInDev( $location )
	{
		...
		\IPS\Db::i()->delete( 'dcudash_templates', array( 'template_master=1 and template_user_created=0 and template_user_edited=0 AND template_location=?', $location ) );
	...
	{
		foreach( \IPS\Db::i()->select( '*', 'dcudash_templates', array( 'template_master=?', 1 ) ) as $template )
		{
			try
			{
	...
	{
		$path = static::_getHtmlPath('dcudash');
	...
	public function getRawTemplates( $app=array(), $location=array(), $group=array(), $returnType=null, $returnThisSetOnly=false )
	{
		...
		if ( ( \IPS\IN_DEV or \IPS\Theme::designersModeEnabled() ) AND ! ( $returnType & static::RETURN_DATABASE_ONLY ) )
		{
			...
			foreach( new \DirectoryIterator( static::_getHtmlPath('dcudash') ) as $location )
			{
				if ( ! in_array( $location->getFilename(), $fixedLocations ) AND $location->isDir() AND mb_substr( $location->getFilename(), 0, 1 ) !== '.' )
				{
					...
					{
						$allowedLocations = array('dash', 'block', 'database');
					}
					else
					{
					...
					foreach( new \DirectoryIterator( static::_getHtmlPath( 'dcudash', $location->getFilename() ) ) as $file )
					{
						if ( $file->isDir() AND mb_substr( $file->getFilename(), 0, 1 ) !== '.' )
						{
							if ( $group === NULL or ! count( $group ) or ( in_array( $file->getFilename(), $group ) ) )
							{
								foreach( new \DirectoryIterator( static::_getHtmlPath( 'dcudash', $location->getFilename(), $file->getFilename() ) ) as $template )
								{
									if ( ! $template->isDir() AND ( mb_substr( $template->getFilename(), -6 ) === '.phtml' or mb_substr( $template->getFilename(), -4 ) === '.css' or mb_substr( $template->getFilename(), -3 ) === '.js' ) )
									{
										...
										$contents  = file_get_contents( static::_getHtmlPath( 'dcudash', $location->getFilename(), $file->getFilename() ) . '/' . $template->getFilename() );
										$key       = \IPS\dcudash\Theme\Template::extractDataFromTag( $contents, 'key' );
										$key       = $key ? $key : \IPS\Http\Url\Friendly::seoTitle( $file->getFilename() . '_' . $title );
										$ogroup    = \IPS\dcudash\Theme\Template::extractDataFromTag( $contents, 'original_group' );
										$params    = \IPS\dcudash\Theme\Template::extractParamsFromTag( $contents );
										$container = NULL;

										if ( in_array( $key, $seenKeys ) )
										{
											$key .= filemtime( static::_getHtmlPath( 'dcudash', $location->getFilename(), $file->getFilename() ) . '/' . $template->getFilename() ) . mt_rand();
										}
										...
										{
											$object = new \IPS\dcudash\Templates;
											$object->key          = $key;
											$object->title        = $title;
											$object->desc         = NULL;
											$object->rel_id       = NULL;
										...
										{
											$results['dcudash'][ $location->getFilename() ][ $file->getFilename() ][ $key ] = array(
												'template_key'            => $key,
												'template_title'          => $title,
												'template_desc'           => NULL,
											...
											{
												unset( $results['dcudash'][ $location->getFilename() ][ $file->getFilename() ][ $key ]['template_content'] );
											}
										}
										else
										{
		...
		else
		{
			...
			foreach( \IPS\Db::i()->select( '*', 'dcudash_templates', implode( " AND ", $where ), 'template_location, template_group, template_key, template_user_edited ASC' ) as $row )
			{
				$rawTemplates[] = $row;
			...
			{
				foreach( \IPS\Db::i()->select( '*', 'dcudash_templates', array( array( 'template_original_group = template_group '), array( \IPS\Db::i()->in( 'template_original_group', $originalGroups ) ) ) ) as $row )
				{
	...
	public function getTemplate( $group, $app=NULL, $location=NULL )
	{
		...
		if ( \IPS\IN_DEV or \IPS\Theme::designersModeEnabled() )
		{
			...
			{
				if ( $app === 'dcudash' AND ! in_array( $location, array( 'admin', 'front', 'global' ) ) )
				{
					$this->templates[ $app ][ $location ][ $group ] = new \IPS\dcudash\Theme\Template( $app, $location, $group );
				}
				else
				{
		...
		else
		{
			...
			if ( ( $app !== 'dcudash' ) OR ( $app === 'dcudash' AND in_array( $location, array( 'admin', 'front', 'global' ) ) ) )
			{
				return \IPS\Theme::i()->getTemplate( $group, $app, $location );
			}

			$key = \strtolower( 'template_dcudash_' .static::makeBuiltTemplateLookupHash( $app, $location, $group ) . '_' . static::cleanGroupName( $group ) );
...
EOF;
	...
	{
		$templates = \IPS\dcudash\Theme::i()->getRawTemplates( $app, $location, $group );
