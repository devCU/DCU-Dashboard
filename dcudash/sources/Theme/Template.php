...
class _Template extends \IPS\Theme\Template
{
	...
	public function __construct( $app, $templateLocation, $templateName )
	{
		...
		{
			$this->sourceFolder = \IPS\ROOT_PATH . "/themes/dcudash/{$templateLocation}/{$templateName}/";
		}
		else if ( \IPS\IN_DEV )
		{
		...
		if ( \IPS\IN_DEV or \IPS\Theme::designersModeEnabled() )
		{
			...
			if( !function_exists( 'IPS\\Theme\\'.$functionName ) )
			{
				...
				if ( $file === NULL or !file_exists( $file ) )
				{
					...
					try
					{
						$template = \IPS\Db::i()->select( '*', 'dcudash_templates', array( 'template_location=? and LOWER(template_group)=? and template_title=?', $this->templateLocation, $this->templateName, $bit ) )->first();
						
						\IPS\dcudash\Theme::makeProcessFunction( $template['template_content'], $functionName, $template['template_params'], TRUE );
					}
				...
				else
				{
					...
					\IPS\dcudash\Theme::makeProcessFunction( $output, $functionName, ( isset( $matches[1] ) ? $matches[1] : '' ), TRUE );
				}
			}
