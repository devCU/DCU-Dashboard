...
class _Categories extends \IPS\Node\Model implements \IPS\Node\Permissions
{
	 ...
	 * @return	NULL|\IPS\dcudash\Categories object
	 */
	...
	static public function isFurlCollision( $path )
	{
		...
		/* _ is here due to IP.Board 3.x using that to denote the dashboards database (eg. dashboards.html/_/category/record-r1/), which we need to redirect from for records and dashboards. */
	...
	public function form( &$form )
	{
		...
		$form->add( new \IPS\Helpers\Form\Text( 'category_furl_name', $this->furl_name, FALSE, array(), function( $val )
		{
			...
			if ( \IPS\Request::i()->category_parent_id == 0 and \IPS\dcudash\Categories::isFurlCollision( $val ) )
			{
			...
			try
			{
				$cat = \IPS\Db::i()->select( '*', 'dcudash_database_categories', array( 'category_database_id=? and category_parent_id=? and category_furl_name=?', \IPS\Request::i()->database_id, \IPS\Request::i()->category_parent_id, $val ) )->first();
			}
		...
		$form->add( new \IPS\Helpers\Form\Node( 'category_parent_id', ( ! $this->id ) ? ( isset( \IPS\Request::i()->parent ) ? \IPS\Request::i()->parent : 0 ) : $this->parent_id, FALSE, array(
			'class'		      => '\IPS\dcudash\Categories' . \IPS\Request::i()->database_id,
			'disabled'	      => false,
		...
		$form->addHeader('dcudash_categories_header_display');
		$templatesList     = array( 0 => \IPS\Member::loggedIn()->language()->addToStack('dcudash_categories_use_database') );
		$templatesDisplay  = array( 0 => \IPS\Member::loggedIn()->language()->addToStack('dcudash_categories_use_database') );
		...
		$form->add( new \IPS\Helpers\Form\Text( 'category_dash_title',  $this->dash_title, FALSE, array(), NULL, NULL, NULL ) );
		...
		$options	= array();
		$fieldClass	= 'IPS\dcudash\Fields' . \IPS\Request::i()->database_id;
	...
	public function formatFormValues( $values )
	{
		...
		{
			\IPS\Lang::saveCustom( 'dcudash', 'content_cat_name_' . $this->id, $values['category_name'] );
		}

		if( isset( $values['category_description'] ) )
		{
			\IPS\Lang::saveCustom( 'dcudash', 'content_cat_name_' . $this->id . '_desc', $values['category_description'] );
		...
		if ( isset( $name ) AND empty( $values['category_furl_name'] ) )
		{
			...
			try
			{
				$cat = \IPS\dcudash\Categories::load( \IPS\Http\Url\Friendly::seoTitle( $name ), 'category_furl_name' );
			...
			if ( \IPS\Request::i()->category_parent_id == 0 and \IPS\dcudash\Categories::isFurlCollision( \IPS\Http\Url\Friendly::seoTitle( $name ) ) )
			{
				$ok = FALSE;
			}
	...
	public function cloneDatabasePermissions()
	{
		...
		$dbPerms  = \IPS\dcudash\Databases::load( $this->database_id )->permissions();
	...
	public function permissions()
	{
		...
		if ( ! $this->_permsMashed )
		{
			...
			$savePerms['perm_2'] = $this->readPermissionMergeWithDash( $savePerms );
	...
	/**
	 * Return least favourable permissions based on category and dash
	 *
	 * @param   array   $perms      Array of perms
	 *
	 * @return string
	 */
	public function readPermissionMergeWithDash( $perms=NULL )
	{
		$database = \IPS\dcudash\Databases::load( $this->database_id );

		/* Now check against the dash */
		if ( $database->dash_id )
		{
			try
			{
				$dash      = \IPS\dcudash\Dashes\Dash::load( $database->dash_id );
				$dashPerms = $dash->permissions();
				$catPerms  = ( $perms ) ? $perms : $this->permissions();

				if ( $dashPerms['perm_view'] === '*' )
				{
					return $catPerms['perm_2'];
				}
				else if ( $catPerms['perm_2'] === '*' )
				{
					return $dashPerms['perm_view'];
				}
				else
				{
					return implode( ',', array_intersect( explode( ',', $dashPerms['perm_view'] ), explode( ',', $catPerms['perm_2'] ) ) );
				}
			}
			catch ( \OutOfRangeException $ex )
			{
	...
	public function url()
	{
		if( $this->_url === NULL )
		{
			if ( \IPS\dcudash\Dashes\Dash::$currentDash and \IPS\dcudash\Databases::load( $this->database_id )->dash_id == \IPS\dcudash\Dashes\Dash::$currentDash->id )
			{
				$dashPath = \IPS\dcudash\Dashes\Dash::$currentDash->full_path;
			}
			else
			{
				try
				{
					$dashPath = \IPS\dcudash\Dashes\Dash::loadByDatabaseId( $this->database_id )->full_path;
				}
				catch( \OutOfRangeException $e )
				{
					return NULL;
			...
			{
				$this->_url = \IPS\Http\Url::internal( "app=dcudash&module=dashes&controller=dash&path=" . $dashPath . '/' . $catPath, 'front', 'content_dash_path', $this->furl_name );
			}
			else
			{
				$this->_url = \IPS\Http\Url::internal( "app=dcudash&module=dashes&controller=dash&path=" . $dashPath, 'front', 'content_dash_path', $this->furl_name );
			}
		}
	...
	public static function urlFromIndexData( $indexData, $itemData, $containerData )
	{
		...
		if ( $recordClass::$dashPath === NULL )
		{
			$recordClass::$dashPath = \IPS\Db::i()->select( array( 'dash_full_path' ), 'dcudash_dashes', array( 'dash_id=?', $recordClass::database()->dash_id ) )->first();
		}
				
		if ( $recordClass::database()->use_categories )
		{
			return \IPS\Http\Url::internal( "app=dcudash&module=dashes&controller=dash&path=" . $recordClass::$dashPath . '/' . $itemData['extra'], 'front', 'content_dash_path', $itemData['extra'] );
		}
		else
		{
			return \IPS\Http\Url::internal( "app=dcudash&module=dashes&controller=dash&path=" . $recordClass::$dashPath, 'front', 'content_dash_path', '' );
		}
	}
	...
	/**
	 * Get Dash Title for use in <title> tag
	 *
	 * @return	string
	 */
	public function dashTitle()
	{
		if ( $this->dash_title )
		{
			return $this->dash_title;
		}
		
		return $this->database()->dashTitle();
	}
	...
	protected function get__titleLanguageKey()
	{
		...
		if ( ! \IPS\dcudash\Databases::load( $this->database_id )->use_categories )
		{
			if ( ! $this->_catTitleLangKey )
			{
				...
				{
					$dash = \IPS\dcudash\Dashes\Dash::loadByDatabaseId( $this->database_id );
					$this->_catTitleLangKey = $dash->_titleLanguageKey;
				}
				catch( \OutOfRangeException $e )
				{
	...
	protected function get__items()
	{
		...
		{
			return \IPS\Db::i()->select('count(*)', 'dcudash_custom_database_' . $this->database_id, array("record_future_date=0 AND category_id=? AND record_approved=1 AND member_id=?", $this->id, \IPS\Member::loggedIn()->member_id) )->first();
		}
	...
	public function setLastComment( \IPS\Content\Comment $comment=NULL )
	{
		...
		{   
			try
			{
				$recordClass  = '\IPS\dcudash\Records' . $this->database_id;
				$commentClass = '\IPS\dcudash\Records\Comment' . $this->database_id;
				$comment      = NULL;
				...
				if ( static::$latestRecordAdded === NULL )
				{
					static::$latestRecordAdded = $recordClass::constructFromData(
						\IPS\Db::i()->select(
							'*',
							'dcudash_custom_database_' . $this->database_id,
				...
				{
					if ( static::$latestRecordAdded->record_comments AND ( $database->_comment_bump & \IPS\dcudash\Databases::BUMP_ON_COMMENT ) )
					{
						...
						else
						{
							try
							{
								$comment = $commentClass::constructFromData( \IPS\Db::i()->select( '*', 'dcudash_database_comments', array( 'comment_record_id=? AND comment_approved=1', \IPS\Db::i()->select( 'primary_id_field', 'dcudash_custom_database_' . $this->database_id, array( 'category_id=? AND record_approved=1', $this->id ), 'record_last_comment DESC', 1 )->first() ), 'comment_date DESC', 1, NULL, NULL, \IPS\Db::SELECT_FROM_WRITE_SERVER )->first() );
							}
		...
		if ( $comment !== NULL and ( $database->_comment_bump & \IPS\dcudash\Databases::BUMP_ON_COMMENT ) )
		{
		...
		$this->records        = \IPS\Db::i()->select( 'COUNT(*)', 'dcudash_custom_database_' . $this->database_id, array( 'record_approved=1 AND record_future_date=0 AND category_id=?', $this->id ), NULL, NULL, NULL, NULL, \IPS\Db::SELECT_FROM_WRITE_SERVER )->first();
		$this->records_queued = \IPS\Db::i()->select( 'COUNT(*)', 'dcudash_custom_database_' . $this->database_id, array( 'record_approved=0 AND category_id=?', $this->id ), NULL, NULL, NULL, NULL, \IPS\Db::SELECT_FROM_WRITE_SERVER )->first();
		$this->records_future = \IPS\Db::i()->select( 'COUNT(*)', 'dcudash_custom_database_' . $this->database_id, array( 'record_future_date=1 AND category_id=?', $this->id ), NULL, NULL, NULL, NULL, \IPS\Db::SELECT_FROM_WRITE_SERVER )->first();
	...
	public function getLastCommentTime()
	{
		if( !$this->can_view_others and !\IPS\Member::loggedIn()->modPermission( 'can_content_view_others_records' ) )
		{
			try
			{
				$select = \IPS\Db::i()->select('record_last_comment', 'dcudash_custom_database_' . $this->database_id, array("record_future_date=0 AND category_id=? AND record_approved=1 AND member_id=?", $this->id, \IPS\Member::loggedIn()->member_id), 'record_last_comment DESC', 1 )->first();
			}
			catch ( \UnderflowException $e )
			{
				return NULL;
	...
	public function lastPost()
	{
		...
		else
		{
			try
			{
				$record = $RecordsClass::constructFromData( \IPS\Db::i()->select('*', 'dcudash_custom_database_' . $this->database_id, array("record_future_date=0 AND category_id=? AND record_approved=1 AND member_id=?", $this->id, \IPS\Member::loggedIn()->member_id), 'record_last_comment DESC', 1 )->first() );
	...
	protected function updateSearchIndexPermissions()
	{
		...
		else
		{
			...
			\IPS\Content\Search\Index::i()->massUpdate( 'IPS\dcudash\Records' . static::database()->_id, $this->_id, NULL, $permissions, NULL, NULL, NULL, NULL, NULL, TRUE );
			\IPS\Content\Search\Index::i()->massUpdate( 'IPS\dcudash\Records\Comment' . static::database()->_id, $this->_id, NULL, $permissions, NULL, NULL, NULL, NULL, NULL, TRUE );
		}
	}
	...
	{
		if ( isset( \IPS\Request::i()->cookie['dcudash_filters'] ) )
		{
			$saved = json_decode( \IPS\Request::i()->cookie['dcudash_filters'], TRUE );
	...
	{
		$cookie = ( isset( \IPS\Request::i()->cookie['dcudash_filters'] ) ) ? json_decode( \IPS\Request::i()->cookie['dcudash_filters'], TRUE ) : array();
		...
		\IPS\Request::i()->setCookie( 'dcudash_filters', json_encode( $cookie ), \IPS\DateTime::create()->add( new \DateInterval( 'P7D' ) ) );
	}
	...
	{
		return $this->dash_title ?: static::database()->metaTitle();
	}
