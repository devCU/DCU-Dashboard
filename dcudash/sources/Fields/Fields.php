...
class _Fields extends \IPS\CustomField implements \IPS\Node\Permissions
{
	 ...
	 */
	public static $databaseTable = 'dcudash_database_fields';
	...
	*/
	 ...
	 */
	protected static $uploadStorageExtension = 'dcudash_Records';
	...
	public static function roots( $permissionCheck='view', $member=NULL, $where=array() )
	{
		...
		if ( ! isset( static::$cache[ static::$customDatabaseId ][ $permissionCheck ] ) )
		{
			...
			{
				if ( ! array_key_exists( $obj->type, static::$additionalFieldTypes ) AND ( ! class_exists( '\IPS\Helpers\Form\\' . mb_ucfirst( $obj->type ) ) AND ! class_exists( '\IPS\dcudash\Fields\\' . mb_ucfirst( $obj->type ) ) ) )
				{
	...
	{
		$key = 'dcudash_fieldids_' . static::$customDatabaseId;
		
		if ( ! isset( \IPS\Data\Store::i()->$key ) )
		{
			\IPS\Data\Store::i()->$key = iterator_to_array( \IPS\Db::i()->select( 'field_id', 'dcudash_database_fields', array( array( 'field_database_id=?', static::$customDatabaseId ) ) )->setKeyField('field_id') );
		}
	 ...
	 * @param	\IPS\Node\Model|NULL		$container			Parent container
	 * @param	int				            $flags				Bit flags
	 * @param   \IPS\dcudash\Records|NULL       $record             The record itself
	 * @return	array
	 */
	public static function fields( $values, $permissionCheck='view', \IPS\Node\Model $container=NULL, $flags=0, \IPS\dcudash\Records $record = NULL )
	{
		$fields        = array();
		$database      = \IPS\dcudash\Databases::load( static::$customDatabaseId );
		...
		foreach( static::roots( $permissionCheck ) as $row )
		{
			...
			if ( $row->unique )
			{
				...
				{
					call_user_func_array( 'IPS\dcudash\Fields' . static::$customDatabaseId . '::validateUnique', array( $val, $row, $record ) );
				};
			}
	 ...
	 * @param	NULL|\IPS\dcudash\Record		$record				Record showing this field
	...
	public function formatForDisplay( $value, $formValue, $type='listing', $record=NULL )
	{
		if ( $type === 'raw' )
		{
			...
			if ( $this->type === 'Item' )
			{
				...
				{
					$results = array();
					$class   = '\IPS\dcudash\Records' . $this->extra['database'];
		...
		else if ( $type === 'thumbs' and $this->type === 'Upload' )
		{
			if ( isset( $this->extra['thumbsize'] ) )
			{
				if ( $this->is_multiple )
				{
					$thumbs = iterator_to_array( \IPS\Db::i()->select( '*', 'dcudash_database_fields_thumbnails', array( array( 'thumb_field_id=? AND thumb_record_id=?', $this->id, $record->_id ) ) )->setKeyField('thumb_original_location')->setValueField('thumb_location') );
					$images = array();
				...
				else
				{
					try
					{
						return (string) \IPS\File::get( static::$uploadStorageExtension, \IPS\Db::i()->select( 'thumb_location', 'dcudash_database_fields_thumbnails', array( array( 'thumb_field_id=? AND thumb_record_id=?', $this->id, $record->_id ) ) )->first() )->url;
					}
					catch( \Exception $e ) { }
				}
			}
		...
		if ( isset( $options[ $type ]['method'] ) AND $options[ $type ]['method'] !== 'simple' )
		{
			if ( in_array( $this->type, static::$mediaFields ) )
			{
				...
				{
					$class = '\IPS\dcudash\Fields\\' . $this->type;
					...
					else
					{
						try
						{
							$value = \IPS\Theme::i()->getTemplate( 'records', 'dcudash', 'global' )->$template( $formValue, $this->extra );
						}
						catch( \Exception $ex )
						{
							$value = $formValue;
				...
				else if ( $options[ $type ]['method'] !== 'none' )
				{
					...
					if ( $this->type === 'Upload' )
					{
						...
						foreach( $files as $file )
						{
							...
							$parsed[] = \IPS\Theme::i()->getTemplate( 'global', 'dcudash', 'front' )->uploadDisplay( \IPS\File::get( static::$uploadStorageExtension, $file ), $record, $downloadUrl );
						}

						$value = implode( " ", $parsed );
					}

					$value = \IPS\Theme::i()->getTemplate( 'records', 'dcudash', 'global' )->fieldBadge( $this->_title, $value, $class );
				}
			}
		}
		else
		{
			...
			$value = \IPS\Theme::i()->getTemplate( 'records', 'dcudash', 'global' )->fieldDefault( $this->_title, $value );
		}
	...
	{
		$fixedFields = \IPS\dcudash\Databases::load( static::$customDatabaseId )->fixed_field_perms;
	...
	public static function setFixedFieldPermissions( $field, $values )
	{
		...
		\IPS\dcudash\Databases::load( static::$customDatabaseId )->fixed_field_perms = $fixedFields;
		\IPS\dcudash\Databases::load( static::$customDatabaseId )->save();
	}
	 ...
	 * @param	\IPS\dcudash\Fields	$field	The field
	 * @param	\IPS\dcudash\Records	$record	The record (if any)
	...
	public static function validateUnique( $val, $field, $record )
	{
		...
		if ( \IPS\Db::i()->select( 'COUNT(*)', 'dcudash_custom_database_' . $database->id, $where )->first() )
		{
	...
	{
		return 'dashes_field_custom_html_' . $type . '_' . $this->id;
	}
	...
	protected function get__badge()
	{
		...
		if ( \IPS\dcudash\Databases::load( $this->database_id )->field_title == $this->id )
		{
			$badge = array( 0 => 'positive ipsPos_right', 1 => 'content_fields_is_title' );
		}
		else if ( \IPS\dcudash\Databases::load( $this->database_id )->field_content == $this->id )
		{
	...
	protected function get__icon()
	{
		...
		else if ( class_exists( '\IPS\dcudash\Fields\\' . mb_ucfirst( $this->type ) ) )
		{
			return NULL;
		}
	...
	public function displayValue( $value=NULL, $showSensitiveInformation=FALSE )
	{
		...
		if ( class_exists( '\IPS\dcudash\Fields\\' . mb_ucfirst( $this->type ) ) )
		{
			/* Is special! */
			$class = '\IPS\dcudash\Fields\\' . mb_ucfirst( $this->type );
		...
		{
			case 'Upload':
				return \IPS\File::get( 'dcudash_Records', $value )->url;
			break;
			case 'Text':
			case 'TextArea':
			...
			case 'Item':
				...
				if ( count( $value ) and isset( $this->extra['database'] ) and $this->extra['database'] )
				{
					...
					return \IPS\Theme::i()->getTemplate( 'global', 'dcudash', 'front' )->basicRelationship( $results );
				}
	...
	public function getButtons( $url, $subnode=FALSE )
	{
		...
		$database = \IPS\dcudash\Databases::load( $this->database_id );
			...
			if ( $this->id != $database->field_title and $this->id != $database->field_content )
			{
				if ( $this->canBeTitleField() )
				{
					$buttons['set_as_title'] = array(
						'icon'	=> 'list-ul',
						'title'	=> 'dcudash_set_field_as_title',
				...
				if ( $this->canBeContentField() )
				{
					$buttons['set_as_content'] = array(
						'icon'	=> 'file-text-o',
						'title'	=> 'dcudash_set_field_as_content',
	...
	public function form( &$form )
	{
		...
		if ( $this->type )
		{
			...
			if ( !$ok )
			{
				\IPS\Output::i()->output .= \IPS\Theme::i()->getTemplate( 'global', 'core', 'global' )->message( \IPS\Member::loggedIn()->language()->addToStack( 'dcudash_field_no_type_warning', NULL, array( 'sprintf' => array( $this->type ) ) ), 'warning', NULL, FALSE );
			}
		}
		...
		if( !$isTitleField )
		{
			...
			foreach( \IPS\dcudash\Databases::databases() as $db )
			{
				...
				{
					$disabled[] = $db->id;
					$databases[ $db->id ] = \IPS\Member::loggedIn()->language()->addToStack( 'dcudash_db_relational_with_name_disabled', NULL, array( 'sprintf' => array( $db->_title ) ) );
				}
			}
			if ( ! count( $databases ) )
			{
				$databases[0] = \IPS\Member::loggedIn()->language()->addToStack('dcudash_relational_field_no_dbs');
				$disabled[] = 0;
			}
		...
		$form->add( new \IPS\Helpers\Form\Radio( 'field_upload_is_image', $this->id ? ( ( isset( $this->extra['type'] ) and $this->extra['type'] == 'image' ) ? 'yes' : 'no' ) : 'yes', TRUE, array(
			'options'	=> array(
				'yes' => 'dcudash_upload_field_is_image',
				'no'  => 'dcudash_upload_field_is_not_image',
		...
		{
			$usingForum = \IPS\dcudash\Databases::load( \IPS\Request::i()->database_id )->forum_record;
			
			if ( ! $usingForum and $this->id )
			{
				$usingForum = \IPS\Db::i()->select( 'COUNT(*)', 'dcudash_database_categories', array( 'category_database_id=? and category_forum_override=1 and category_forum_record=1', $this->id ) );
			}
		...
		if ( $this->id AND ( $this->id == \IPS\dcudash\Databases::load( static::$customDatabaseId )->field_title OR $this->id == \IPS\dcudash\Databases::load( static::$customDatabaseId )->field_content ) )
		{
			$isTitleOrContent = TRUE;

			if ( $this->id == \IPS\dcudash\Databases::load( static::$customDatabaseId )->field_title )
			{
				$form->addMessage( 'field_display_opts_title', 'ipsMessage ipsMessage_info' );
			}

			if ( $this->id == \IPS\dcudash\Databases::load( static::$customDatabaseId )->field_content )
			{
		...
		$form->add( new \IPS\Helpers\Form\Text( 'field_key', $this->id ? $this->key : FALSE, FALSE, array(), function( $val )
		{
			try
			{
				...
				throw new \InvalidArgumentException('dcudash_field_key_not_unique');
			}
		...
		{
			$displayOptions[ $id ] = $listingOptions[ $id ] = \IPS\Theme::i()->getTemplate( 'records', 'dcudash', 'global' )->fieldBadge( \IPS\Member::loggedIn()->language()->addToStack('dcudash_badge_label'), \IPS\Member::loggedIn()->language()->addToStack('dcudash_badge_value'), 'ipsBadge_style' . $id );
			$listingToggles[ $id ] = array( 'field_display_listing_json_badge_right' );
		}

		$displayOptions['simple'] = $listingOptions['simple'] = \IPS\Theme::i()->getTemplate( 'records', 'dcudash', 'global' )->fieldDefault( \IPS\Member::loggedIn()->language()->addToStack('dcudash_badge_label'), \IPS\Member::loggedIn()->language()->addToStack('dcudash_badge_value' ) );
		...
		$form->add( new \IPS\Helpers\Form\Codemirror( 'field_display_listing_json_custom', ( isset( $displayJson['listing']['html'] ) ? $displayJson['listing']['html'] : NULL ), FALSE, array( 'placeholder' => '{label}: {value}' ), function( $val )
        {
            ...
            {
	            throw new \LogicException('dcudash_field_error_bad_syntax');
            }
		...
		/* Display where? */
		$form->add( new \IPS\Helpers\Form\Radio( 'field_display_display_json_where', ( isset( $displayJson['display']['where'] ) ? $displayJson['display']['where'] : 'top' ), FALSE, array( 'options' => array( 'top' => 'dcudash_field_display_top', 'bottom' => 'dcudash_field_display_bottom' ) ), NULL, NULL, NULL, 'field_display_display_json_where' ) );
		...
		\IPS\Output::i()->globalControllers[]  = 'dcudash.admin.fields.form';
		\IPS\Output::i()->jsFiles  = array_merge( \IPS\Output::i()->jsFiles, \IPS\Output::i()->js( 'admin_fields.js', 'dcudash' ) );

		\IPS\Output::i()->title  = ( $this->id ) ? \IPS\Member::loggedIn()->language()->addToStack('dcudash_edit_field', FALSE, array( 'sprintf' => array( $this->_title ) ) ) : \IPS\Member::loggedIn()->language()->addToStack('dcudash_add_field');
	}
	...
	{
		static::$contentDatabaseTable = 'dcudash_custom_database_' . static::$customDatabaseId;
		...
		{
			\IPS\Lang::saveCustom( 'dcudash', static::$langKey . '_' . $this->id, $values['field_title'] );
		}
		
		if ( isset( $values['field_description'] ) )
		{
			\IPS\Lang::saveCustom( 'dcudash', static::$langKey . '_' . $this->id . '_desc', $values['field_description'] );
			unset( $values['field_description'] );
		}
		
		if ( array_key_exists( 'field_validator_error', $values ) )
		{
			\IPS\Lang::saveCustom( 'dcudash', static::$langKey . '_' . $this->id . '_validation_error', $values['field_validator_error'] );
		...
		if ( isset( $values['field_type'] ) AND $values['field_type'] === 'Item' )
		{
			...
			{
				throw new \LogicException( \IPS\Member::loggedIn()->language()->addToStack('dcudash_relational_field_no_db_selected') );
			}
	...
	public function delete( $skipDrop=FALSE )
	{
		...
		\IPS\Db::i()->delete( 'dcudash_database_fields_reciprocal_map', array( 'map_origin_database_id=? AND map_field_id=? ', static::$customDatabaseId, $this->id ) );
	 ...
	 * @param   \IPS\dcudash\Records|NULL   $record     The record
	 * @param	int				        $flags		Bit flags
	 * @return \IPS\Helpers\Form\FormAbstract
	 */
	public function buildHelper( $value=NULL, $customValidationCode=NULL, \IPS\dcudash\Records $record = NULL, $flags=0 )
	{
		if ( class_exists( '\IPS\dcudash\Fields\\' . mb_ucfirst( $this->type ) ) )
		{
			/* Is special! */
		...
		switch ( mb_ucfirst( $this->type ) )
		{
			...
			case 'Item':
				...
				$options['class']    = '\IPS\dcudash\Records' . $this->extra['database'];
				break;
			case 'Number':
			...
			switch( mb_ucfirst( $this->type ) )
			{
				...
				case 'TextArea':
					if ( $this->unique )
					{
						...
						{
							call_user_func_array( 'IPS\dcudash\Fields' . static::$customDatabaseId . '::validateUnique', array( $val, $field, $record ) );
							
							return call_user_func( 'IPS\dcudash\Fields' . static::$customDatabaseId . '::validateInput_' . $field->id, $val );
						};
					}
					else
					{
						$customValidationCode = 'IPS\dcudash\Fields' . static::$customDatabaseId . '::validateInput_' . $this->id;
					}
				break;
			}
		}
