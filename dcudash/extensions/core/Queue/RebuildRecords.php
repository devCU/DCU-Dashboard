...
class _RebuildRecords
{
	...
	public function preQueueData( $data )
	{
		...
		try
		{
			$data['count'] = (int) \IPS\Db::i()->select( 'COUNT(*)', 'dcudash_custom_database_' . $databaseId )->first();
		}
		catch( \Exception $ex )
		{
	...
	public function run( $data, $offset )
	{
		...
		{
			foreach ( \IPS\Db::i()->select( '*', 'dcudash_custom_database_' . $databaseId, NULL, 'primary_id_field asc', array( $offset, $this->rebuild ) ) as $row )
			{
				...
				if ( $fixImage )
				{
					if ( $record->record_image and file_exists( \IPS\ROOT_PATH . '/uploads/' . $record->record_image ) )
					{
						try
						{
							$record->record_image = (string) \IPS\File::create( 'dcudash_Records', $record->record_image, file_get_contents( \IPS\ROOT_PATH . '/uploads/' . $record->record_image ) );
						}
						catch ( \Exception $e )
						{
				...
				if ( $fixHtml )
				{
					$fields = iterator_to_array( \IPS\Db::i()->select( '*', 'dcudash_database_fields', array( 'field_database_id=? AND field_type=? AND field_html=1', $databaseId, 'Editor' ) ) );
	...
	public function getProgress( $data, $offset )
	{
		...
		return array( 'text' => \IPS\Member::loggedIn()->language()->addToStack('rebuilding_dcudash_database_records', FALSE, array( 'sprintf' => array( $title ) ) ), 'complete' => $data['count'] ? ( round( 100 / $data['count'] * $offset, 2 ) ) : 100 );
	}	
}